...
- **注記欄（annotation lanes）**: `SystemGeometry` に `lanes`（上の欄: `mark` / `tempo` / `text` / `ottava` / `technique` の順、必要なものだけ）と `annotationTop`・`annotationBottom`・`dynamicsBaseline`（強弱欄）を持つ。段の内容は `annotationTop` だけ下へ平行移動し、`unitHeight` に上下の欄が増やした高さを含める。上の欄は、段の内容の上端の空き（セクション名・括弧・演奏順序記号がなく、コード名・高い音より上）に最大 14 まで重ね、`annotationTop` は欄の高さからその重なりを引いた値になる。強弱記号は一番下の五線のインクの下端（小節の歌詞、五線より下のインライン音符・スタッカート・接続の弧、リードシートでは歌詞）から一定の間隔の基準線に置き、段の下の余白に収まらない分だけ `annotationBottom` として高さを足す。メロディのある段（`melody` / `leadSheet`）は `lift` を持つ: `melodyInkTop`（`layout.ts`）がメロディ譜表の見出しより下のインクの最上端を `melodyStaff.ts` の配置規則（小節ごとの臨時記号の状態、連桁グループと符幹の向き、連符番号、下向き符幹の音から上に描く H/P・スラーの弧、タイ、アーティキュレーション、フェルマータ・ビブラート・ベンド・息継ぎ、装飾音）から少し余裕を持って見積もり、`lift = max(0, floor(inkTop − 8 − (33 + 0.25 × chordSize)))` とする。`renderMelodyStaff` はセクション名・コード名・カギ線・演奏指示記号を `<g class="melody-header" transform="translate(0, lift)">` に入れ、段の内容は `annotationTop − lift` だけ平行移動する。`contentHeight` は `lift` だけ小さくなり、`unitHeight = contentHeight + annotationTop + annotationBottom` は保たれる。上の欄の重なりは持ち上げ後の内容（見出しは元の位置）で計算するので規則は変わらない。リズムのみの段は `lift = 0`。これによりページ分割（残り高さの計算）が注記を含めた高さで行われ、隣の段と重ならない。欄の中の文字列は小節ごとの x に置き、前の要素の右端より左に来ないようにずらす。
