...
- **テストファイル**:
  ...
  - `tests/unit/render-advanced.test.ts`: 構造的な段区切り、全段で揃う第1小節線、転調（打ち消し・臨時記号のリセット・リズムのみの段の `Key:`）、拍子記号、注記欄と改ページ、注記欄を五線に近づける配置（コード名の上の空きへの重ね、強弱記号を一番下の五線・歌詞・低い音のすぐ下に置く）、同じ小節前の複数強弱記号が重ならないこと、XML エスケープ、オッターヴァ、奏法記号（高音でもベンドの矢印が上向き）、連符、従来の楽譜の描画が変わらないこと（`origin/main` の描画との一致）
  - `tests/unit/grammar.test.ts`: 新しいヘッダー・スコアイベント・連符・奏法の TextMate 規則
- **テスト実行結果**: **415 / 415 件 PASS** (0 failures)
