...
- **テストファイル**:
  ...
  - `tests/unit/capo.test.ts`: 汎用推論（0〜12 の候補、出現回数の重み、同点時の小さいカポ、不正カポの拒否）、移調と `NOTE_NAMES` の綴り、弾きやすさの計算式と段階の境界、ソース変換（カポ差分、ヘッダー挿入、カポ 0、CRLF・コメント・メロディ・歌詞・繰り返しのバイト保持、長さ指定の保持、ラベル付きコード・定義衝突の拒否、再解析）、有効 DSL のドリフトなし、カポバーの描画、コードトークンとヘッダーのソース位置
...
- **テスト実行結果**: **274 / 274 件 PASS** (0 failures)
