...
| **ローカル音源採譜 (Audio MIR, 実験的)** | `wasm/crates/audio-mir/`, `src/audioMir/` | 🧪 実験的 | 4/4 のみ。テンプレート分類器によるコード推定（学習モデルは未導入。倍音の差し引き {3,6} と 7th ゲートで maj7/7 への偏りを抑制）、拍・ダウンビート、8/12/16 グリッドのストローク位置。既知の限界: 約 100 Hz 未満のベースは 8192 点 FFT の分解能を超える／ドラムの無い音源で半分テンポになりやすい（#56）／ファンク・ジャズ等のテンションの多い和音のルート精度が低い／近接音程のうなりやキックの残響が偽アタックになりうる／ダウン・アップは位置からの推定／小節をまたぐサステイン・ゴースト・アルペジオは未対応 |
...
- **Rust 単体テスト (`cargo test --manifest-path wasm/Cargo.toml`)**: **55 / 55 件 PASS**（倍音の多い音色 3 種で三和音の 7th 誤認がベースライン比で減り、本物の 7th を失わないことを含む）。生成した合成音のみを使用し、次の項目を確認している。
  - WAV 形式の受理と拒否
  - STFT、HPSS、クロマ
...
- **コード精度の評価（#52）**: データはローカル専用でコミットしない。手順は testing.md §5。報告用データでの #50 ベースライン比は次のとおり。
  - GuitarSet 伴奏（演奏者 03〜05、90 曲）: ルート 48.2% → 53.4%、完全一致 30.4% → 43.5%
  - GM 合成マルチ楽器セット（36 曲）: ルート 90.0% → 91.1%、完全一致 79.9% → 89.4%。6 楽器すべてでルート・完全一致とも横ばいか改善
