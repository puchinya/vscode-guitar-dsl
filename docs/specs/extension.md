...
- **実行と進捗表示**:
  ...
  2. Gemini API（`@google/genai` の `interactions.create`）へ、次の複数パスを同じモデル・同じ動画コンテキストで順に送信する。後続パスは `previous_interaction_id` で連結し、各リクエストで JSON Schema を毎回指定する。
     - **ベースラインパス**: 固定プロンプト、YouTube URL、Music IR JSON Schema を送る。タイトル・アーティスト・BPM・キー・セクション／小節構成・メロディ・歌詞の正とする。
     - **ハーモニー精緻化パス**: 各小節のコード変化を、16 分グリッド位置（`tick16` 0..15、小節頭の 0 は必須）と、実音（発音）コード候補 1〜3 件（信頼度付き）で取得する。コード長は拡張機能側で決定論的に算出する（Gemini に音価文字列を計算させない）。小節線をまたぐ食いは、前の小節の位置で次のコードとして表す。
     - **曖昧コード検証パス**（条件付き）: 最上位の信頼度が 0.78 未満、または 1 位と 2 位の差が 0.18 未満の変化点がある場合に限り、1 回だけ実行する。Gemini は提示された候補からのみ選択する。候補外の回答や失敗時は、最上位候補を採用して続行する。
     - **グルーヴ観測パス**（伴奏パターンが「自動」の場合のみ）: 小節ごとにグリッド（8／12／16。「8ビート優先」は 8、「16ビート優先」は 16 に固定）、奏法（ストローク／アルペジオ／伸ばし）、打点位置、任意のアクセント位置、前小節からの延長有無、信頼度を取得する。
     - ハーモニー／グルーヴ結果のセクション数・小節数がベースラインと一致しない等の構造不正は、エラー内容と正しい小節構成を添えて **1 回だけ** 再要求する。再要求でも不正ならエラーとする。
  3. ベースラインのレスポンスに構造バリデーションとセマンティックバリデーションを実施する（BPM 30..300、4/4 拍子、コード名・音高の妥当性、歌詞・音節）。メロディのみ、各小節 4 拍への休符補完／切り詰めを許可する。
  4. 拡張機能内で決定論的に次を行う:
     - **ストローク最適化**: 定番の伴奏パターンプリセットと観測リズムを候補とし、セクション単位で総コスト（打点・アクセントの差分、観測信頼度のペナルティ、小節間の切り替えコスト）を最小化して、各小節のリズムを選択する。観測リズムのストローク方向は、全打点が乗る最も粗いグリッド上の腕の振り子（表拍＝ダウン、裏拍＝アップ、3 連は拍ごとにダウン・アップ・ダウン）で付与する。伴奏パターンが明示指定された場合は、全小節にそのパターンを適用する。
     - **カポ決定**: 手動指定時は指定値をそのまま使う。自動時はカポ 0〜7 のうち、押さえやすさの総合スコアが最小のものを選ぶ（同点は低いカポ）。コードはカポ分だけ移調したプレイ形で出力し、`key:` は実音キーのまま出力する。
     - 最終 IR を厳格に検証する（各小節のコード・リズムが合計 4 拍。拍数合わせのためにコードやストロークを補完・捏造しない）。
  5. 検証済み IR を純粋シリアライザにより決定論的 GuitarDSL テキストへ変換する。カポ設定（`capo:`）、同一パターンの繰り返し（`%` 記号）、メロディ音符ごとの音節歌詞（`lyr:`）を活用して出力する。
  6. `parseGuitarDsl` により構文検証を実施し、エラー診断が 0 件であることを確認。
  7. 成功時のみ `workspace.openTextDocument({ language: 'guitardsl', content })` を呼び出し、`showTextDocument` で新規エディタとして開く。
  8. 既存ファイルの上書きや自動保存は行わない。
- **エラー処理**:
  - 不正な URL、認証・クォータ・ネットワーク・API エラー、非公開動画、無効な IR、非対応の拍子（4/4 以外）等の失敗時はドキュメントを作成せず、既存ファイルを一切変更しない。
  - API キー、モデルの生レスポンス、スタックトレースを含まない簡潔に分類されたエラーメッセージを通知する。上記の構造不正時の 1 回の再要求を除き、自動リトライは行わない。
  - ハーモニー精緻化・グルーヴ観測の失敗時は、ベースラインのコード・リズムに黙ってフォールバックせず、エラーとする。曖昧コード検証パスの失敗のみ、最上位候補で続行する。
