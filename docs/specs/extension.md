...
- **処理フロー**:
  ...
  3. 描画する DSL は**有効 DSL**（§4.4、§4.5）とする。対象ドキュメントがプレビュー中で初心者モードまたはカポの一時変更が有効な場合はプレビューと完全に同じ変換後テキスト（初心者モードが優先）、それ以外はドキュメントのテキストそのもの。プレビューと同一のページ SVG（§4.3）を生成し、拡張機能内でベクター PDF に変換して出力する（1 シート = 1 PDF ページ）。
...
4. **カポバー**（ツールバーの下の 2 段目。§4.4）:
   ...
   - 一時変更が解除されたときの警告。
   - `初心者モード` の ON/OFF と、ON のときの `バレーコード`（`許可する` / `使用しない`）選択（§4.5）。ON の間はカポ選択・スコア・`DSLに適用` が初心者モードの評価と変換に切り替わり、コードの置き換えの概要（例 `F → Fmaj7`）を表示する。
...
- **タイ・連符**: 符尾間のタイ弧線、および8分・16分音符の符桁（ビーム）または符尾（フラッグ）を正確に描画。3連符と一般の連符（`8{5:4}` 等）は括弧と数字を描画する。
- **拍子・調・スコアイベント**: 小節ごとに有効な拍子・調（`docs/specs/guitardsl-syntax.md` §16）で描画する。拍子・調の変更は新しい段から始め、拍子記号・打ち消しを含む調号（リズムのみの段では `Key:` 表示）を描画する。テンポ・フィール・リハーサルマーク・テキスト・オッターヴァ・奏法範囲は段の上の注記欄、強弱は段の下の強弱欄に描画し、注記欄の分だけ段の高さを増やす。
- **奏法**: ハンマリング・プリング（弧＋H/P）、スライド、グリッサンド、チョーキング（矢印＋量）、ビブラート、スタッカート、テヌート、フェルマータ、ブレス、装飾音符、スラー、パームミュート・レットリング（範囲）、アルペジアートを描画する。段をまたぐ接続線・範囲は段ごとに分けて描く。
- プレビューと PDF は同じ SVG 描画を使うため、上記はすべて PDF にも同じく出力される。
- **改ページ**:
...
### 4.5 初心者モード（プレビュー）
初心者モードは、鳴る響きをカポで保ちながら、弾きやすいカポ位置と簡単なコードへの置き換えを組み合わせて提案・表示・適用する。DSL の構文は増やさず、状態は DSL・設定・ワークスペースに保存しない（§4.4 と同じくプレビュー中のドキュメントに対する一時状態）。

- **バレーコード**: `許可する`（allow）/ `使用しない`（forbid）。`使用しない` は絶対条件で、変換後の楽譜に描画されるダイアグラム（`chord` 定義、なければ標準の押さえ方。分数コードは上のコードの標準の押さえ方）にセーハがあるコードは使わない。条件を満たせない場合に `許可する` へ自動で切り替えることはない。
- **カポ候補**: カポ 0〜12 のそれぞれについて、§4.4 と同じ響きを保つカポ変更（`docs/specs/guitardsl-syntax.md` §4.4）を行い、変更できないカポは初心者モードでも選べない。変更後の各コードについて、そのまま・または次の置き換えから選ぶ。1 つでも使えるコードがないカポは選べない。
- **置き換えの規則**（これ以外は行わない。ルート、長三和音／短三和音の性質は変えない）:

| 元 | 置き換え先 | ペナルティ |
|---|---|---:|
| `maj7` / `7` / `add9` / `6` | 長三和音 | 1 |
| `m7` / `m6` | `m` | 1 |
| `9` | `7`（1）/ 長三和音（2） | 1 / 2 |
| `m9` | `m7`（1）/ `m`（2） | 1 / 2 |
| `maj9` | `maj7`（1）/ 長三和音（2） | 1 / 2 |
| `7sus4` | `sus4` | 1 |
| `dim7` | `dim` | 1 |
| 分数コード `X/B` | `X`（ベースを省く。上の置き換えと組み合わせ可、ペナルティは加算） | 1 |
| 長三和音 / `m`（`使用しない` でそのままのコードにセーハがある場合だけ） | `maj7` / `m7` | 2 |

  - ラベル付きコード（`C@label`）は置き換えない。置き換え先と同じ名前のラベルなし `chord` 定義がファイルにある置き換えは使わない。`chord` 定義は変更・移調・削除しない（使われなくなる定義は警告のみ）。
  - 各コードは「押さえ方のコスト（`docs/design/architecture.md` の弾きやすさの計算式）＋ペナルティ」が最小のものを選ぶ（同点はペナルティが小さいもの、そのままのコード、規則の順）。
- **推奨カポ**: 選べるカポのうち、曲の押さえ方のコスト（§4B.3 と同じ式を置き換え後のコードで計算）に出現回数で重み付けしたペナルティの平均を足した値が最小のもの。同点は置き換えた出現回数が少ない、置き換えたコードの種類が少ない、小さいカポの順。表示する弾きやすさ（スコア・段階）は押さえ方のコストだけから計算し、ペナルティは含めない。
- **ON にしたとき**: `バレーコード` は `使用しない`、カポは自動（推奨カポ）。§4.4 の一時変更は解除する。
- **カポを選んだとき**: そのカポに固定して置き換えを計算し直す。`バレーコード` を変えたときは自動に戻し、推奨カポを選び直す。
- **OFF にしたとき**: 初心者モードの状態を解除し、ドキュメントのテキストそのものを描画する（以前の §4.4 の一時変更は戻さない）。
- **有効 DSL**: ON の間は、ドキュメントの**現在の**テキストからカポ変更とコード名の置き換えを行った DSL。カポ値とコード名以外（`chord` 定義、コメント、空白、改行コード、長さ指定、メロディ、歌詞、`key`、リズム）は変えない。プレビュー、プレビューの PDF 保存、同じドキュメントに対する `guitardsl.exportPdf` はこの同一の文字列を使う。変換は毎回ドキュメントのテキストから計算し、積み重ねない。
- ドキュメントを編集すると、自動なら推奨カポから、固定カポならそのカポで計算し直す。解がなくなった・変換できなくなった場合は初心者モードを解除し、元の DSL を描画して警告を表示する。プレビューが別のドキュメントに切り替わる、プレビューを閉じる、対象ドキュメントを閉じると解除する。
- **DSLに適用**: その時点のドキュメントのテキストから現在の選択（バレーコード、自動／固定カポ）で変換をやり直し、カポ値と置き換えたコード名を 1 回の編集（元に戻す 1 回で戻る）で適用する。適用後は初心者モードを解除する。適用できない場合は理由を表示し、ドキュメントを変更しない。
...
### 4B.4 初心者モード セクション
§4.5 の初心者モードの設定と適用を行う。設定は DSL に保存しない。
- `バレーコード`（`許可する` / `使用しない`、初期値 `使用しない`）とカポ（`自動` / 0〜12、初期値 `自動`）。
- 推奨カポ、現在の弾きやすさ（ドキュメントそのまま、§4B.3）、変更後の弾きやすさ（§4.5 の表示用スコア）。
- カポ 0〜12 の候補一覧（スコア、段階、推奨、選べない理由）。選べる候補をクリックするとそのカポに固定する。
- コードの対応（元 → 変更後）。置き換えたコードは印を付けて区別する。
- 選べない理由・警告（使われなくなる可能性のある `chord` 定義）。
- **DSLに適用**: §4.5 の適用と同じ。変換結果がドキュメントと同じ場合は無効。

### 4B.5 移調 セクション
曲全体の実音移調（`docs/specs/guitardsl-syntax.md` §4.5）と、移調後のカポの選び方を組み合わせて適用する。新しいコマンドは追加しない（`guitardsl.editScoreSettings` のセクション）。
- **移調量**: `-11`〜`+11` 半音（初期値 0）。
- **移調後のキー**: 冒頭の `key:` を解析できる場合、移調後のキーの一覧から選べ、選択は移調量と同期する。解析できない場合は移調量だけで指定する。
- 表示: 現在の（鳴る）キー、移調後の（鳴る）キー、現在のカポ、コードの対応（元の書かれたコード → 移調・カポ適用後の書かれたコード）、警告（使われなくなる `chord` 定義、推奨カポがない場合）、移調できない理由。
- **カポ**: `そのまま`（移調後もカポを変えない、初期値）/ `推奨`（移調後の DSL に対する §4B.3 の推奨カポ）/ `指定`（0〜12）。`推奨` でコードが1つもない場合は現在のカポのままとし、警告を表示する。
- 変換の順序: ドキュメントのテキストを実音移調し、その結果に対してカポを選んで §4.4 の響きを保つカポ変更を行う。
- **DSLに適用**: その時点のドキュメントのテキストから移調とカポ変更をやり直し（Webview の表示内容・テキストは使わない）、1 回の編集（元に戻す 1 回で戻る）として適用する。適用できない場合は理由を表示し、ドキュメントを変更しない。変換結果がドキュメントと同じ場合は無効。
...
- **スコアイベントシンボル**:
  - `SymbolKind.Event`
  - スコアイベント行（`@key: D`、`@tempo: 132`、`@time: 7/8`、`@mark: B` 等）を、イベント名と値を含むラベルで表示する。セクション内のイベントはセクションの子、セクションより前のイベントは最上位に置く。
...
| 警告 | 小節の合計拍数がその小節の拍子の長さと一致しない（§8.2.1, §12.3, §16.2, §16.3） |
| エラー | 不正な拍子・拍のグループ、不正な `feel` / `pickup`（§16.2, §16.3, §16.6） |
| エラー | 不正なスコアイベント（未知の名前・不正な値、§16.1） |
| 警告 | 最後の小節より後のスコアイベント（§16.1） |
| エラー | 拍子の異なる小節の `%` による繰り返し（§8.7） |
| エラー | 不正な連符比（§8.2.1） |
| 警告 | 完結しない連符グループ（§8.2.1） |
| エラー | 不正な奏法、音高を必要とする奏法の休符・スラッシュへの指定（§8.4, §12.2.1） |
| 警告 | 接続先のない奏法・装飾音符、スラーの入れ子・対応なし・閉じられていない（§12.2.1） |
